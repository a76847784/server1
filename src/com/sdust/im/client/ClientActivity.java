...
import com.sdust.im.utils.RegisterUtils;
...
public class ClientActivity {
	...
	public void regist(TranObject tran) {
		...
		try {
			//加入身份验证
			RegisterUtils.RegisterMicroInnovationAPI(user);
	...
	public void searchFriendBySimilarity(TranObject tran) {
		User user=(User) tran.getObject();
		
		Objects.requireNonNull(user, "[相似度查找]user未传送");
		Objects.requireNonNull(user.getAccount(), "[相似度查找]账号未传送");
		Objects.requireNonNull(user.getPhoto(), "[相似度查找]照片未传送");
		
		//采用相似度交友
		ArrayList<User> list=UserDao.selectFriendBySimilarity(user);
		
//		System.out.println((String) tran.getObject());
		System.out.println("[相似度查找]发送客户端查找的好友列表...");
	 ...
	 * 查找朋友
	 * 
	 * 相似度交友前台需要传user对象，其它传String对象即可
	 */
