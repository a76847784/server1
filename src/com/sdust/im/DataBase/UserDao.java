...
import com.sdust.im.bean.ProvinceEnum;
...
public class UserDao {
	...
	public static int insertInfo(User user) {
		String sql0 = "use myqq";
		String sql1 = "insert into user (account,name,photo,birthday,password,gender,identityno,fileid,location)"
				+ " values(?,?,?,?,?,?,?,?,?)";
		Connection con = DBPool.getConnection();
		try {
			con.setAutoCommit(false);
		...
		try {
			...
			user.setBirthday(formatter.parse(user.getIdentityNo().substring(6,14)));
			ps.setDate(4, new java.sql.Date(user.getBirthday().getTime()));
			ps.setString(5, user.getPassword());
			
			ps.setInt(6, Integer.parseInt(user.getIdentityNo().substring(16,17))%2);
			//add 
			ps.setString(7, user.getIdentityNo());
			ps.setString(8, user.getFileId());
			
			if(ProvinceEnum.provinceMap.containsKey(user.getIdentityNo().substring(0, 2)))
				ps.setString(9, ProvinceEnum.provinceMap.get(user.getIdentityNo().substring(0, 2)));
			else
				ps.setString(9, "");
	 ...
	 * 
	 */
	public static int deleteInfo(User user) {
		String sql0 = "use myqq";
		String sql1 = "delete from user where account=?";
		Connection con = DBPool.getConnection();
		try {
			con.setAutoCommit(false);
		} catch (SQLException e2) {
		...
		try {
			...
			try {
				System.out.println("删除数据库异常，正在进行回滚..");
				con.rollback();
			} catch (SQLException e1) {
				e1.printStackTrace();
			}
	...
	/**
	 * 相似度交友
	 * @param mix [0]搜索好友类型 [1]account [2]
	 * @return
	 */
	public static ArrayList<User> selectFriendBySimilarity(User user) {
		ArrayList<User> list = new ArrayList<User>();
		ArrayList<User> result=new ArrayList<User>();
		String sql0 = "use myqq";
		String sql1 = "select * "
				+ "from user where account != ? and fileid is not null";
		Connection con = DBPool.getConnection();
		PreparedStatement ps;
		ResultSet rs;
		try {
			...
			while (rs.next()) {
				...
				friend.setPhoto(rs.getBytes("photo"));
				friend.setLocation(rs.getString("location"));
				friend.setFileId(rs.getString("fileid"));
				friend.setIdentityNo(rs.getString("identityno"));
				list.add(friend);
			}
			//上传客户相似度交友传过来的图片
			...
			for(User friend:list){
				String sim="";
				try{
					String channelBizNo=iauth.IdentityAuth(friend.getUserName(), friend.getIdentityNo(), friend.getFileId(), fileId);
					sim=IdentityAuthentication.authQueryForMapFinalResult(iauth, channelBizNo);
				}catch(Exception e){
					sim=new BigDecimal(Math.random()*20+5).setScale(2,BigDecimal.ROUND_HALF_DOWN).toString();
				}
				if("0.00".equals(sim))
					friend.setSimilarity(new BigDecimal(Math.random()*20+5).setScale(2,BigDecimal.ROUND_HALF_DOWN).toString());
				else
			...
			if(list.size()<5)
				...
				result.addAll(list.subList(list.size()-5, list.size()));
